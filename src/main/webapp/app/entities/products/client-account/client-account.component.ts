...
export class ClientAccountComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.clientAccountService
      ...
      .subscribe(
        (res: HttpResponse<IClientAccount[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInClientAccounts(): void {
    this.eventSubscriber = this.eventManager.subscribe('clientAccountListModification', () => this.loadPage());
  }

  delete(clientAccount: IClientAccount): void {
    const modalRef = this.modalService.open(ClientAccountDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IClientAccount[] | null, headers: HttpHeaders, page: number): void {
